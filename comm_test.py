...
# Reformater la colonne 'Date' au format mois/jour/année
df_tri["Date"] = df_tri["Date"].dt.strftime('%m/%d/%Y')
...
ath_df = ath_df.rename(columns={'Close': 'ATH', 'Date': 'Date ATH'})  # Renommer la colonne 'Date' en 'Date ATH'
...
# Convertir les colonnes 'Fermeture' et 'Date ATH' en objets datetime
result_df['Fermeture'] = pd.to_datetime(result_df['Fermeture'], format='%m/%d/%Y')
result_df['Date ATH'] = pd.to_datetime(result_df['Date ATH'], format='%m/%d/%Y')

# Ajouter une nouvelle colonne avec la différence de jours
result_df['Diff_Jours'] = (result_df['Fermeture'] - result_df['Date ATH']).dt.days
...
# Paramètres
start_date = pd.to_datetime('01/01/2024', format='%m/%d/%Y')  # Date de début du modèle
...
df_tri['Date'] = pd.to_datetime(df_tri['Date'], format='%m/%d/%Y')
...
for symbole in result_df['Symbole']:
    ...
    if len(X) > 0 and len(y) > 0:
        ...
        result_df.loc[result_df['Symbole'] == symbole, 'Estimation J+30'] = future_value[0][0]

# Estimation de rendement en pourcentage
estimation = ((result_df['Estimation J+30'] - result_df['Dernière Close']) / result_df['Dernière Close']) * 100
